...
object Cardinals {
  ...
  trait TreeSeqRec[N <: Nat] extends NatTypeRec1[AnyRef] {

    type OnZero[+A] = Tree[N, A]

    type OnSucc[P <: Nat, T[+_] <: AnyRef, +A] =
      Tree[N, P#Rec1[AnyRef, TreeSeqRec[S[N]] , A]]

  }

  type TreeSeq[N <: Nat, K <: Nat, +A] = K#Rec1[AnyRef, TreeSeqRec[N], A]

  //============================================================================================
  // AUXILLARY TYPES
  //

  trait CardinalTreeRec extends NatTypeRec1[AnyRef] {

    type OnZero[+A] = Tree[_0, A]

    type OnSucc[P <: Nat, T[+_] <: AnyRef, +A] =
      P#Rec1[AnyRef, CardinalTreeRec, Tree[S[P], A]]

  }

  type CardinalTree[N <: Nat, +A] = N#Rec1[AnyRef, CardinalTreeRec, A]
  type CardinalNesting[N <: Nat, +A] = CardinalTree[N, Nesting[N, A]]

  type Cardinal[N <: Nat, +A] = DimSeq[CardinalNesting, N, A]

  def NilCard[A] : IncSeq[CardinalNesting, _0, _0, A] = IncNil[CardinalNesting, _0, A]()
