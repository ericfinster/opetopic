...
abstract class Renderer[T, A](implicit isNumeric : Numeric[T]) {
  ...
  def renderComplex[N <: Nat](cmplx : Complex[N, A]) : Option[Complex[N, BoxType]] =
    (new NatCaseSplit {
      ...
      type Out[M <: Nat] = Option[Complex[M, BoxType]]
  ...
  def renderNesting[N <: Nat](nst : Nesting[S[N], A], canvas : NestingCanvas, lvs : Tree[N, LayoutMarker])
    ...
    nst match {
      ...
      case Box(a, c) => {
        ...
        val (leafCount : Int, leavesWithIndices : Tree[N, (LayoutMarker, Int)]) = zipWithIndex(lvs)
        ...
        def verticalPass(tr : Tree[S[N], Nesting[S[N], A]]) : Option[(LayoutMarker, Tree[S[N], Nesting[S[N], BoxType]])] = {
          ...
          object verticalRecursor extends GraftRecursor[N, Nesting[S[N], A], ResultType] {

            def leafRec(addr : Address[N]) : Option[ResultType] =
              for {
                leafMarkerWithIndex <- valueAt(leavesWithIndices, addr) // Have to do a traverse with an address ...
              } yield {
            ...
            def nodeRec(sn : Nesting[S[N], A], sh : Tree[N, ResultType]) : Option[ResultType] = {
