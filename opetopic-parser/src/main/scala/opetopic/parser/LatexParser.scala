/**
  * LatexParser.scala - Latex Parsing
  * 
  * @author Tom Tung
  * @version 0.1 
  */
 ...
 */
class LatexParser(val input: ParserInput) extends Parser {
  ...
  //     (if (name.last.isLetter) &(!("a" - "z" | "A" - "Z")) else MATCH)

  //   names.foldLeft(MISMATCH)((rule, s) => rule | getRule(s)) ~> self
  // }
  ...
  //   matchCommandNames(escapeNames) ~> translateEscape(_)
  // }
